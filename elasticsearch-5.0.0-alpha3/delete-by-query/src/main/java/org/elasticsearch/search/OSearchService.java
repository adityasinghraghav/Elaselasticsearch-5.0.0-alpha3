...
public class OSearchService extends AbstractLifecycleComponent<SearchService> {
    ...
    @Inject
    public OSearchService(Settings settings, NodeSettingsService nodeSettingsService, ClusterService clusterService, IndicesService indicesService,IndicesWarmer indicesWarmer, ThreadPool threadPool,
        ...
        indicesService.indicesLifecycle().addListener(new IndicesLifecycle.Listener() {
            ...
            public void afterIndexClosed(Index index, Settings indexSettings) {
                ...
                IndexMetaData idxMeta = OSearchService.this.clusterService.state().metaData().index(index.getName());
    ...
    class SearchSettingsListener implements NodeSettingsService.Listener {
        ...
        public void onRefreshSettings(Settings settings) {
            final TimeValue maybeNewDefaultSearchTimeout = settings.getAsTime(OSearchService.DEFAULT_SEARCH_TIMEOUT, OSearchService.this.defaultSearchTimeout);
            if (!maybeNewDefaultSearchTimeout.equals(OSearchService.this.defaultSearchTimeout)) {
                logger.info("updating [{}] from [{}] to [{}]", OSearchService.DEFAULT_SEARCH_TIMEOUT, OSearchService.this.defaultSearchTimeout, maybeNewDefaultSearchTimeout);
                OSearchService.this.defaultSearchTimeout = maybeNewDefaultSearchTimeout;
            }
        }
    }
    ...
    private void parseTemplate(ShardSearchRequest request, SearchContext searchContext) {
        ...
        if (request.template() != null) {
            ExecutableScript executable = this.scriptService.executable(request.template(), ScriptContext.Standard.SEARCH, searchContext, Collections.<String, String>emptyMap());
