...
import org.elasticsearch.search.controller.OdoscopeSearchPhaseController;
...
class OdoscopeSearchQueryThenFetchAsyncAction extends OdoscopeAbstractSearchAsyncAction<QuerySearchResultProvider> {

    final AtomicArray<FetchSearchResult> fetchResults;
    final AtomicArray<IntArrayList> docIdsToLoad;

    OdoscopeSearchQueryThenFetchAsyncAction(ESLogger logger, SearchServiceTransportAction searchService,
    ...
    private void finishHim() {
        threadPool.executor(ThreadPool.Names.SEARCH).execute(new ActionRunnable<SearchResponse>(listener) {
            ...
            public void doRun() throws IOException {
                ...
                OdoscopeSearchPhaseController o = new OdoscopeSearchPhaseController();
                ...
                if (!queryResults.isEmpty())
                {
                    ...
                    if((new HashMap<>(internalResponse2.profile()).size()!=0))
                    {
                        shardResults = new InternalProfileShardResults(new HashMap<>(internalResponse2.profile()));
                    }
                    ...
                    InternalSearchHits hits = o.Odoscope((InternalSearchHits)internalResponse2.hits(), internalResponse2.hits().totalHits());
