...
import org.elasticsearch.plugin.deletebyquery.MyParser;
...
public class RestDeleteByQueryAction extends BaseRestHandler {
    ...
    public RestDeleteByQueryAction(Settings settings, RestController controller, Client client) {
        ...
        controller.registerHandler(POST, "/{index}/_hello", this);
        controller.registerHandler(GET, "/{index}/{type}/_hello", this);
        controller.registerHandler(POST, "/{index}/{type}/_hello", this);
        controller.registerHandler(GET, "/_hello/template", this);
        controller.registerHandler(POST, "/_hello/template", this);
        controller.registerHandler(GET, "/{index}/_hello/template", this);
        controller.registerHandler(POST, "/{index}/_hello/template", this);
        controller.registerHandler(GET, "/{index}/{type}/_hello/template", this);
        controller.registerHandler(POST, "/{index}/{type}/_hello/template", this);

        RestExistsAction restExistsAction = new RestExistsAction(settings, controller, client);
        controller.registerHandler(GET, "/_hello/exists", restExistsAction);
        controller.registerHandler(POST, "/_hello/exists", restExistsAction);
        controller.registerHandler(GET, "/{index}/_hello/exists", restExistsAction);
        controller.registerHandler(POST, "/{index}/_hello/exists", restExistsAction);
        controller.registerHandler(GET, "/{index}/{type}/_hello/exists", restExistsAction);
        controller.registerHandler(POST, "/{index}/{type}/_hello/exists", restExistsAction);
    }

    @Override
    public void handleRequest(final RestRequest request, final RestChannel channel, final Client client) {
        SearchRequest searchRequest = new SearchRequest();
        RestDeleteByQueryAction.parseSearchRequest(searchRequest, request, parseFieldMatcher, null);

        MyParser p = new MyParser();
        p.parseSource(searchRequest.source(), null );

        String s = String.valueOf(p.getLength());
        String t = String.valueOf(p.getOffset());

        SavedValues.setS(p.getLength());
        SavedValues.setF(p.getOffset());

        SimpleRequestBuilder builder = new SimpleRequestBuilder(client);
        SimpleRequest simple = builder.request();
        simple.setSearchRequest(searchRequest);
        SearchResponse response = client.execute(SimpleAction.INSTANCE, simple).actionGet();
    ...
    public static SearchSourceBuilder parseSearchSource(RestRequest request) {
        ...
        MyParser p = new MyParser();
        p.parseSource(request.content(), null );
        SavedValues.setS(p.getLength());
        SavedValues.setF(p.getOffset());
